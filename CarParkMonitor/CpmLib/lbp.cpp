...
void lbp::OLBP_(const Mat& src, Mat& dst) {
	dst = Mat_<_Tp>(src.rows-2, src.cols-2);
...
void lbp::ELBP_(const Mat& src, Mat& dst, int radius, int neighbors) {
	...
	dst = Mat_<_Tp>::zeros(src.rows-2*radius, src.cols-2*radius);
	for(int n=0; n<neighbors; n++) {
		...
		for(int i=radius; i < src.rows-radius;i++) {
			for(int j=radius;j < src.cols-radius;j++) {
				...
				dst.at<_Tp>(i-radius,j-radius) += ((t > src.at<_Tp>(i,j)) && (abs(t-src.at<_Tp>(i,j)) > std::numeric_limits<float>::epsilon())) << n;
			}
		}
	}
}
